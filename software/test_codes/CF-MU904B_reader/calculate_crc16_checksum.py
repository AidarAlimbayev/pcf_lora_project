...
def calculate_crc16_msb(data):
    crc16_func = crcmod.predefined.Crc('crc-16')
    crc16_func.update(data)
    crc16_result = crc16_func.crcValue
    return crc16_result.to_bytes(2, byteorder='big')

# Example usage:
data = b"Hello, world!"
crc16_msb_result = calculate_crc16_msb(data)

print(f"CRC-16 MSB result for '{data.decode()}': {crc16_msb_result.hex()}")
