...
def Collect_data_CSV(cow_id, weight_finall, type_scales): # Collocting datat into CSV, in the future must be in SQLite
    try:
        ...
        cur = conn.cursor() 
        cur.execute("DROP TABLE IF EXISTS COW")
        table_cow = """ CREATE TABLE COW (
                    cowID INT NOT NULL,
                    weight_final TEXT,
                    date_now TEXT,
                    type_scales TEXT
                ); """
        cur.execute(table_cow)
        SQL_Statement = '''INSERT INTO COW (cowID, weight_final, date_now, type_scales) VALUES (?, ?, ?, ?)'''
